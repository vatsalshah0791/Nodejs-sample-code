...
export default class ReadDoctorsAssignedController {
    constructor(
    ...
    run = async (
        ...
        try {
            const readCollaboratorsAssignedApplicationResult = await this.readCollaboratorAssignedApplication.run({ ...payload.params, selectIdAppRole: [3] })
            req.apiResponse = {
