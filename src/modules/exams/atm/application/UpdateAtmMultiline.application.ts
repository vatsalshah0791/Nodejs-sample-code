...
export default class UpdateAtmMultilineApplication {
    constructor(private readonly atmRepository: AtmRepository) {}
    async run (data: UpdateAtmMultilineInterface): Promise<Result> {
        ...
        const updateAtmMultioptionsResult = await this.atmRepository.updateAtmMultiline(data)
        if(!updateAtmMultioptionsResult) {
            const response: Result = {
                success: false,
                statusCode: 500,
