...
export default class VerifyPlanExpirationDateAdminApplication {
    constructor(private readonly planRepository: PlanRepository) {}
    async run(data: VerifyPlanExpirationDateInterface): Promise<Result> {
        const VerifyPlanExpirationDateAdminResult = await this.planRepository.verifyPlanExpirationDateAdmin(data)
        if(!VerifyPlanExpirationDateAdminResult) {
